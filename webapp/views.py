...
#####################################--subscriber data classes------###########################################################
class subscriber_GetCountUtility(APIView):
    #Read file
    def get_data(self):
        APP_ROOT = os.path.dirname(os.path.abspath(__file__))
        file_path = os.path.join(APP_ROOT, 'subscriber_data.xlsx')
        data = pd.read_excel(file_path)
        #print('read subcribe data',data)
        return data

    def get_count(self):
        df = self.get_data()
        df_count = df.groupby('StateName').count().reset_index()
        df_count = df_count[['State', 'SubscriberCode']]
        df_count = df_count.rename(columns={'FullName': 'Count'})
    ...
    def get_top10_count(self):
        df = self.get_data()
        top10_StateCount=df.groupby(["State"]).size().reset_index(name="count")
        top10_StateCount = top10_StateCount.sort_values(by='count', ascending=False)
        top10_StateCount = top10_StateCount.head(10).reset_index(drop=True)
        #self.top10_StateCount=self.get_data()
       # df_get_top_10 = data.groupby(["StateName"]).size().reset_index(name="count")
    ...
    def Taluka_Count(self, state_name, district):
        ...
        # state = input("Enter the StateName:")
        # district = input("Enter the DistrictName:")
        df1 = df.loc[(df["State"] == state_name) & (df["District"] == district)]
        df2 = df1.groupby(["Taluka"]).size().reset_index(name="Talukacount")
        return df2


class subscriber_PlotStateCount(APIView):
    def plot_graph(self):
        obj = subscriber_GetCountUtility()
        df_count = obj.get_count()
        fig = px.bar(data_frame=df_count, x='State', y='Count', color="State")
        return fig
    def Pie_Chart(self):
        obj = subscriber_GetCountUtility()
        top10_StateCount = obj.get_top10_count()
...
class subscriber_GetUniqueStates(APIView):
    def get(self, request):
        obj = subscriber_GetCountUtility()
        df = obj.get_data()
        # df = self.get_data()
        ...
        for state in unique_states:
            ...
            res[state] = list(df_state['District'].unique())    
        # res['Districts'] = unique_districts
        # res = json.dumps(res)
...
class subscriber_StateCount(APIView):
    def get(self, request):
        obj = subscriber_PlotStateCount()
        fig = obj.plot_graph()
        graphJSON = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
        return Response(graphJSON)
...
class subscriber_top10StateCount(APIView):
    def get(self,request):
        obj = subscriber_PlotStateCount()
        top_10_df = obj.Pie_Chart()
        labels = list(top_10_df['State'])
        data = list(top_10_df['count'])
        #fig = px.pie(self.top10_StateCount, count='pop', names='Statename')
        # graphJSON1 = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
        backgroundColor = ['#ffff00','#ff0000','#00ff00','#ffa500','#0000ff','#FFE4C4','#f3cfbb','#808080','#ffc0cb','#E6E6FA']
...
class subscriber_TalukaWiseCount(APIView):
    def post(self,request):
        data = json.loads(request.body)
        # print(data, type(data))
        state_name = data['State']
